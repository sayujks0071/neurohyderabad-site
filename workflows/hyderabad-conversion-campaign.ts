...
async function createCampaignTrackerSheet(): Promise<string | null> {
  ...
  const args = ['sheets', 'spreadsheets', 'create', '--json', `{"properties": {"title": "${title}"}}`];
...
async function addHeaderRow(spreadsheetId: string) {
  ...
  const args = ['sheets', 'spreadsheets', 'values', 'append', '--params', `{"spreadsheetId": "${spreadsheetId}", "range": "${range}", "valueInputOption": "USER_ENTERED"}`, '--json', jsonPayload];
