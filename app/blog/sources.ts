export type SourceEntry = {
  label: string;
  href: string;
};

export const BLOG_SOURCES: Record<string, SourceEntry[]> = {
  "brain-aneurysm-vs-migraine-warning-signs-hyderabad": [
    {
      label: "Brain Aneurysm Foundation – Warning Signs",
      href: "https://www.bafound.org/warning-signs-symptoms/",
    },
    {
      label: "Mayo Clinic – Brain Aneurysm",
      href: "https://www.mayoclinic.org/diseases-conditions/brain-aneurysm/symptoms-causes/syc-20361483",
    },
    {
      label: "National Health Service (UK) – Subarachnoid Hemorrhage",
      href: "https://www.nhs.uk/conditions/subarachnoid-haemorrhage/",
    },
  ],
  "world-stroke-day-2025-hyderabad-stroke-code": [
    {
      label: "World Stroke Organization – Global Stroke Fact Sheet 2023",
      href: "https://www.world-stroke.org/assets/downloads/WSO_Global_Stroke_Fact_Sheet.pdf",
    },
    {
      label: "World Health Organization India – Stroke",
      href: "https://www.who.int/india/health-topics/stroke",
    },
    {
      label: "The Lancet Global Health – Stroke Burden in India 1990–2019",
      href: "https://www.thelancet.com/journals/langlo/article/PIIS2214-109X(22)00067-3/fulltext",
    },
    {
      label: "Indian Stroke Association – Acute Ischaemic Stroke Guidelines 2022",
      href: "https://journals.sagepub.com/doi/full/10.1177/09731499211021223",
    },
    {
      label: "Indian Council of Medical Research – Stroke Surveillance & Prevention Program",
      href: "https://main.icmr.nic.in/content/stroke",
    },
  ],
  "endoscopic-discectomy-cost-hyderabad": [
    {
      label: "American Association of Neurological Surgeons – Lumbar Disc Herniation",
      href: "https://www.aans.org/en/Patients/Neurosurgical-Conditions-and-Treatments/Lumbar-Disc-Herniation",
    },
    {
      label: "National Health Service (UK) – Lumbar decompression surgery overview",
      href: "https://www.nhs.uk/conditions/lumbar-decompression-surgery/",
    },
    {
      label: "Insurance Regulatory and Development Authority of India – Day-care treatment guidelines",
      href: "https://irdai.gov.in/admincms/cms/Uploadedfiles/consumers/Daycare.pdf",
    },
  ],
  "endoscopic-spine-surgery-cost-hyderabad": [
    {
      label: "North American Spine Society – Coverage policy recommendations for endoscopic spine procedures",
      href: "https://www.spine.org/PolicyPractice/CoverageRecommendations",
    },
    {
      label: "National Health Service (UK) – Minimally invasive spinal surgery (MISS)",
      href: "https://www.nhs.uk/conditions/back-surgery/",
    },
  ],
  "day-care-endoscopic-spine-surgery-eligibility": [
    {
      label: "American Society of Anesthesiologists – Ambulatory surgery selection criteria",
      href: "https://www.asahq.org/standards-and-guidelines",
    },
    {
      label: "Insurance Regulatory and Development Authority of India – Day-care treatment guidelines",
      href: "https://irdai.gov.in/admincms/cms/Uploadedfiles/consumers/Daycare.pdf",
    },
  ],
  "day-care-spine-surgery-insurance-hyderabad": [
    {
      label: "Insurance Regulatory and Development Authority of India – Policyholder rights handbook",
      href: "https://irdai.gov.in/ADMINCMS/cms/Uploadedfiles/Publications/Handbook_Policyholder.pdf",
    },
    {
      label: "Guidelines on Standard Health Insurance Products – Government of India",
      href: "https://www.pmjay.gov.in/sites/default/files/2020-11/Guidelines-on-Standard-Health-Insurance.pdf",
    },
  ],
  "endoscopic-vs-microdiscectomy-hyderabad": [
    {
      label: "Journal of Spine Surgery – Comparative outcomes of endoscopic vs. microdiscectomy",
      href: "https://jss.amegroups.org/article/view/4953/html",
    },
    {
      label: "American Association of Neurological Surgeons – Minimally invasive spine surgery",
      href: "https://www.aans.org/en/Patients/Neurosurgical-Conditions-and-Treatments/Minimally-Invasive-Spine-Surgery",
    },
  ],
  "mvd-vs-radiosurgery-trigeminal-neuralgia": [
    {
      label: "Cleveland Clinic – Trigeminal Neuralgia Treatment Options",
      href: "https://my.clevelandclinic.org/health/diseases/15601-trigeminal-neuralgia",
    },
    {
      label: "National Center for Biotechnology Information – Microvascular decompression vs stereotactic radiosurgery",
      href: "https://www.ncbi.nlm.nih.gov/pmc/articles/PMC6012129/",
    },
  ],
  "return-to-work-after-endoscopic-discectomy-hyderabad": [
    {
      label: "National Health Service (UK) – Lumbar decompression surgery recovery",
      href: "https://www.nhs.uk/conditions/lumbar-decompression-surgery/recovery/",
    },
    {
      label: "European Spine Journal – Early return-to-work guidance after minimally invasive lumbar surgery",
      href: "https://link.springer.com/article/10.1007/s00586-018-5722-8",
    },
  ],
  "sciatica-pain-management-hyderabad": [
    {
      label: "National Institute of Neurological Disorders and Stroke – Sciatica Fact Sheet",
      href: "https://www.ninds.nih.gov/health-information/disorders/sciatica",
    },
    {
      label: "National Institute for Health and Care Excellence – Low back pain and sciatica management",
      href: "https://www.nice.org.uk/guidance/ng59",
    },
  ],
  "spine-health-maintenance-hyderabad": [
    {
      label: "American Academy of Orthopaedic Surgeons – Spine health basics",
      href: "https://orthoinfo.aaos.org/en/staying-healthy/fit-ness-and-your-back/",
    },
    {
      label: "Harvard Medical School – Back care tips for spine health",
      href: "https://www.health.harvard.edu/pain/caring-for-your-back",
    },
  ],
  "spine-surgery-recovery-timeline-hyderabad": [
    {
      label: "Hospital for Special Surgery – Spine surgery recovery guidelines",
      href: "https://www.hss.edu/conditions_spine-surgery-recovery.asp",
    },
    {
      label: "National Health Service (UK) – Recovering from back surgery",
      href: "https://www.nhs.uk/conditions/back-surgery/recovery/",
    },
  ],
  "spinal-fusion-cost-hyderabad": [
    {
      label: "American Academy of Orthopaedic Surgeons – Spinal Fusion surgical overview",
      href: "https://orthoinfo.aaos.org/en/treatment/spinal-fusion/",
    },
    {
      label: "North American Spine Society – Coverage recommendations for spinal fusion",
      href: "https://www.spine.org/PolicyPractice/CoverageRecommendations/AboutCoverageRecommendations",
    },
  ],
  "brain-tumor-surgery-cost-hyderabad": [
    {
      label: "National Cancer Institute – Brain Tumor Treatment (PDQ®) Patient Version",
      href: "https://www.cancer.gov/types/brain/patient/brain-treatment-pdq",
    },
    {
      label: "American Cancer Society – Brain and Spinal Cord Tumor Treatment Options",
      href: "https://www.cancer.org/cancer/brain-spinal-cord-tumors-adults/treating.html",
    },
  ],
  "awake-craniotomy-guide": [
    {
      label: "Johns Hopkins Medicine – Awake Brain Surgery Overview",
      href: "https://www.hopkinsmedicine.org/health/treatment-tests-and-therapies/awake-brain-surgery",
    },
    {
      label: "Mayo Clinic – Awake Brain Surgery (Craniotomy)",
      href: "https://www.mayoclinic.org/tests-procedures/awake-brain-surgery/about/pac-20384577",
    },
  ],
  "disc-replacement-vs-fusion": [
    {
      label: "North American Spine Society – Lumbar artificial disc replacement coverage recommendations",
      href: "https://www.spine.org/PolicyPractice/CoverageRecommendations",
    },
    {
      label: "National Center for Biotechnology Information – Systematic review comparing disc replacement and fusion",
      href: "https://www.ncbi.nlm.nih.gov/pmc/articles/PMC6029347/",
    },
  ],
  "cervical-radiculopathy-treatment-hyderabad": [
    {
      label: "American Academy of Orthopaedic Surgeons – Cervical Radiculopathy (Pinched Nerve)",
      href: "https://orthoinfo.aaos.org/en/diseases--conditions/cervical-radiculopathy-pinched-nerve/",
    },
    {
      label: "National Institute of Neurological Disorders and Stroke – Cervical Radiculopathy Information",
      href: "https://www.ninds.nih.gov/health-information/disorders/cervical-radiculopathy",
    },
  ],
  "sciatica-pain-treatment-hyderabad": [
    {
      label: "American Academy of Orthopaedic Surgeons – Sciatica (Lumbar Radiculopathy)",
      href: "https://orthoinfo.aaos.org/en/diseases--conditions/sciatica-lumbar-radiculopathy/",
    },
    {
      label: "National Institute of Neurological Disorders and Stroke – Sciatica Information",
      href: "https://www.ninds.nih.gov/health-information/disorders/sciatica",
    },
  ],
  "brain-spine-surgeon-banjara-hills": [
    {
      label: "Yashoda Hospital – Official Website and Location Information",
      href: "https://www.yashodahospitals.com/",
    },
    {
      label: "Hyderabad Medical Tourism – Healthcare Infrastructure",
      href: "https://www.hyderabad.gov.in/",
    },
  ],
  "endoscopic-cervical-spine-surgery-hyderabad": [
    {
      label: "North American Spine Society – Coverage policy recommendations for endoscopic cervical procedures",
      href: "https://www.spine.org/PolicyPractice/CoverageRecommendations",
    },
    {
      label: "National Health Service (UK) – Cervical radiculopathy and neck pain treatment",
      href: "https://www.nhs.uk/conditions/neck-pain-and-stiff-neck/",
    },
    {
      label: "Journal of Spine Surgery – Outcomes of endoscopic cervical foraminotomy",
      href: "https://jss.amegroups.org/article/view/4826/html",
    },
  ],
  "spine-tumor-surgery-hyderabad": [
    {
      label: "American Association of Neurological Surgeons – Spinal Tumors",
      href: "https://www.aans.org/patients/conditions-treatments/spinal-tumors/",
    },
    {
      label: "National Cancer Institute – Spine Tumor Overview",
      href: "https://www.cancer.gov/types/spinal-cord-tumors",
    },
    {
      label: "Spine Universe – Types of Spinal Tumors",
      href: "https://www.spineuniverse.com/conditions/spinal-tumors",
    },
  ],
  "cauda-equina-syndrome-warning-signs-hyderabad": [
    {
      label: "American Academy of Orthopaedic Surgeons – Cauda Equina Syndrome",
      href: "https://orthoinfo.aaos.org/en/diseases--conditions/cauda-equina-syndrome/",
    },
    {
      label: "American Association of Neurological Surgeons – Cauda Equina Syndrome",
      href: "https://www.aans.org/Patients/Neurosurgical-Conditions-and-Treatments/Cauda-Equina-Syndrome",
    },
    {
      label: "NHS UK – Cauda Equina Syndrome Overview",
      href: "https://www.nhs.uk/conditions/cauda-equina-syndrome/",
    },
  ],
  "cervical-disc-replacement-vs-fusion-acdf-hyderabad": [
    {
      label: "North American Spine Society – Cervical Artificial Disc Replacement",
      href: "https://www.spine.org/KnowYourBack/Treatments/Surgical/CervicalArtificialDiscReplacement",
    },
    {
      label: "American Academy of Orthopaedic Surgeons – Cervical Disc Replacement",
      href: "https://orthoinfo.aaos.org/en/treatment/cervical-disc-replacement/",
    },
    {
      label: "Spine-Health – ACDF vs. Cervical Disc Replacement",
      href: "https://www.spine-health.com/treatment/spine-surgery/acdf-vs-cervical-disc-replacement",
    },
  ],
  "degenerative-disc-disease-treatment-hyderabad": [
    {
      label: "North American Spine Society – Degenerative Disc Disease",
      href: "https://www.spine.org/KnowYourBack/Conditions/DegenerativeDiscDisease",
    },
    {
      label: "American Academy of Orthopaedic Surgeons – Cervical and Lumbar Degenerative Disc Disease",
      href: "https://orthoinfo.aaos.org/en/diseases--conditions/lumbar-degenerative-disc-disease/",
    },
    {
      label: "Arthritis Foundation – Degenerative Disc Disease",
      href: "https://www.arthritis.org/diseases/degenerative-disc-disease",
    },
  ],
  "scoliosis-treatment-options-in-hyderabad": [
    {
      label: "Scoliosis Research Society – Patient Information",
      href: "https://www.srs.org/patients-and-families",
    },
    {
      label: "American Association of Neurological Surgeons – Scoliosis",
      href: "https://www.aans.org/Patients/Neurosurgical-Conditions-and-Treatments/Scoliosis",
    },
    {
      label: "Mayo Clinic – Scoliosis Symptoms & Causes",
      href: "https://www.mayoclinic.org/diseases-conditions/scoliosis/symptoms-causes/syc-20350716",
    },
  ],
};

export function getBlogSources(slug: string): SourceEntry[] {
  return BLOG_SOURCES[slug] ?? [];
}

// Export sources object for direct access
export const sources = BLOG_SOURCES;
