...
import { extractDischargeSummaryPdfTextInSandbox } from "@/lib/discharge-summary/pdfExtract";
import { interpretDischargeSummaryText } from "@/lib/discharge-summary/interpret";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (contentType.includes("application/json")) {
      ...
      const extraction = await extractDischargeSummaryPdfTextInSandbox(buffer);
    ...
    // Interpret text
    const analysis = await interpretDischargeSummaryText(text);
    ...
    console.error("[Discharge Summary Analyzer] Error:", error);
