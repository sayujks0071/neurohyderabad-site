...
import { slack } from '@/src/lib/slack'; // Slack Client
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // 🔔 Notify Slack if Human Handoff is detected or implied
    const shouldNotifySlack =
      body.message.toLowerCase().includes('human') ||
      body.message.toLowerCase().includes('support') ||
      body.message.toLowerCase().includes('doctor') ||
      aiResponse.includes('call us directly') ||
      aiResponse.includes('I apologize');

    if (shouldNotifySlack) {
      // Fire and forget - don't block response
      slack.notify(`📢 *Chat Activity Alert*
*User:* ${body.message}
*AI Response:* ${aiResponse.substring(0, 200)}...
*Reason:* Potential Human Handoff / Fallback detected`).catch(e => console.error('Slack notify failed:', e));
    }
