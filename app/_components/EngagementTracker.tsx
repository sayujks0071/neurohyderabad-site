...
export default function EngagementTracker({
  ...
  useEffect(() => {
    ...
    let lastProcessedActivity = 0;
    const ACTIVITY_THROTTLE_MS = 1000;

    const handleActivity = () => {
      const now = Date.now();
      
      // ⚡ Bolt: Fast timestamp-based throttle to completely remove setTimeout overhead
      // on high-frequency events (scroll, mousemove, keypress)
      if (now - lastProcessedActivity < ACTIVITY_THROTTLE_MS) {
        return;
      }

      lastProcessedActivity = now;
