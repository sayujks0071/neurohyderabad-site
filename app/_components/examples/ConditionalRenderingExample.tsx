    ...
    <div className="space-y-8">
      ...
      {layoutVariant === 'compact' ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-[var(--color-primary-50)] p-4 rounded">Compact Layout - Column 1</div>
          <div className="bg-[var(--color-primary-50)] p-4 rounded">Compact Layout - Column 2</div>
        </div>
      ) : (
        <div className="space-y-4">
          <div className="bg-[var(--color-primary-50)] p-4 rounded">Standard Layout - Full Width</div>
        </div>
      )}
      ...
      {showVideoSection && (
        <section className="bg-[var(--color-background)] p-6 rounded-lg">
          ...
          <div className="aspect-video bg-[var(--color-border)] rounded flex items-center justify-center">
            <p className="text-[var(--color-text-secondary)]">Video Player Placeholder</p>
          </div>
        </section>
      )}

      {showPricingTable && (
        <section className="bg-[var(--color-surface)] border rounded-lg p-6">
      ...
      {enableLiveChat && (
        <div className="fixed bottom-4 right-4 bg-[var(--color-success)] text-white p-4 rounded-full shadow-lg cursor-pointer hover:bg-[var(--color-success)]">
          💬 Chat
        </div>
      )}
