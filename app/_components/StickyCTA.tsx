...
export default function StickyCTA({ className = '' }: StickyCTAProps) {
  ...
  return (
    <div 
      className={`fixed bottom-0 left-0 right-0 bg-white/70 backdrop-blur-lg border-t border-white/20 shadow-xl z-50 p-4 ${className}`}
      role="region"
          ...
          <div className="ml-4 flex flex-wrap gap-3" role="group" aria-label="Contact actions">
            {config.buttons.map((button) => (
              <Link 
                ...
                className={`${buttonToneClasses[button.intent]} font-semibold rounded-xl transition-all duration-300 hover:scale-[1.02] active:scale-[0.98] py-3 px-6 min-h-[44px] text-sm flex items-center justify-center focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2`}
