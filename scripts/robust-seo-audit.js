...
/**
 * Robust SEO Audit for drsayuj.info
 * Handles real-world sitemap structure
 */
...
const MAX_PAGES = 50; // Start with smaller sample
...
// Get URLs from sitemap using curl
function getSitemapUrls() {
  try {
    console.log('🔍 Fetching sitemap URLs...');
    
    // Get sitemap content
    const sitemapContent = execSync(`curl -s "${SITE_URL}/sitemap-0.xml"`, { encoding: 'utf8' });
...
// Analyze page using curl
function analyzePage(url) {
  try {
    console.log(`📊 Analyzing: ${url}`);
    
    // Get page content
    const html = execSync(`curl -s -L "${url}"`, { encoding: 'utf8' });
...
async function runRobustAudit() {
  console.log('🚀 Starting robust SEO audit for drsayuj.info\n');
  ...
  try {
    ...
    console.log(`📊 Analyzing ${keyPages.length} key pages...`);
    const pageResults = [];
    ...
    const jsonFile = path.join(reportDir, `robust-seo-audit-${timestamp}.json`);
    const mdFile = path.join(reportDir, `robust-seo-audit-${timestamp}.md`);
...
function generateMarkdownReport() {
  ...
  return `# Robust SEO Audit Report - drsayuj.info
